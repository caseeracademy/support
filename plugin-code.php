...
class Caseer_Academy_API_Gateway {
    ...
    public function __construct() {
        ...
        // Webhook Triggers
        add_action('woocommerce_order_status_changed', array($this, 'trigger_order_status_webhook'), 10, 4);
        add_action('woocommerce_new_order', array($this, 'trigger_new_order_webhook'), 10, 2);
    }
    ...
    public function trigger_order_status_webhook($order_id, $old_status, $new_status, $order) {
        ...
        foreach ($order->get_items() as $item) {
            $items_data[] = array('name' => $item->get_name(), 'quantity' => $item->get_quantity());
        }
    ...
    public function register_routes() {
        ...
        register_rest_route($namespace, '/user/create', array('methods' => 'POST', 'callback' => array($this, 'create_user'), 'permission_callback' => array($this, 'check_secret_key_permission')));
        register_rest_route($namespace, '/admin/latest-students', array('methods' => 'GET', 'callback' => array($this, 'get_latest_students'), 'permission_callback' => array($this, 'check_secret_key_permission')));
        register_rest_route($namespace, '/admin/reset-password', array('methods' => 'POST', 'callback' => array($this, 'admin_reset_password'), 'permission_callback' => array($this, 'check_secret_key_permission')));
        register_rest_route($namespace, '/admin/search-users', array('methods' => 'GET', 'callback' => array($this, 'admin_search_users'), 'permission_callback' => array($this, 'check_secret_key_permission')));
        register_rest_route($namespace, '/admin/student/(?P<id>\d+)', array('methods' => 'GET', 'callback' => array($this, 'admin_get_student_details'), 'permission_callback' => array($this, 'check_secret_key_permission')));
        register_rest_route($namespace, '/courses', array('methods' => 'GET', 'callback' => array($this, 'get_all_courses'), 'permission_callback' => '__return_true'));
        register_rest_route($namespace, '/my-courses', array('methods' => 'GET', 'callback' => array($this, 'get_my_courses'), 'permission_callback' => array($this, 'user_is_logged_in')));
        register_rest_route($namespace, '/course/(?P<id>\d+)', array('methods' => 'GET', 'callback' => array($this, 'get_course_details'), 'permission_callback' => array($this, 'user_is_logged_in')));
        register_rest_route($namespace, '/lesson/(?P<id>\d+)', array('methods' => 'GET', 'callback' => array($this, 'get_lesson_details'), 'permission_callback' => array($this, 'user_is_logged_in')));
        register_rest_route($namespace, '/user/profile', array('methods' => 'GET', 'callback' => array($this, 'get_user_profile'), 'permission_callback' => array($this, 'user_is_logged_in')));
        register_rest_route($namespace, '/initiate-payment', array('methods' => 'POST', 'callback' => array($this, 'initiate_payment'), 'permission_callback' => array($this, 'user_is_logged_in')));
    }
    ...
    public function admin_get_student_details(WP_REST_Request $request) {
         if (!function_exists('wc_get_orders') || !function_exists('tutor_utils')) {
            return new WP_REST_Response(array('message' => 'WooCommerce or Tutor LMS is not active.'), 500);
        }
        ...
        if ($enrolled_courses_query && $enrolled_courses_query->have_posts()) {
            while ($enrolled_courses_query->have_posts()) {
                ...
                $enrolled_courses_data[] = array('id' => $course_id, 'title' => get_the_title(), 'thumbnail_url' => get_the_post_thumbnail_url($course_id, 'medium_large'));
            }
            wp_reset_postdata();
        }
        ...
        foreach ($orders as $order) {
            $items = array();
            foreach ($order->get_items() as $item) {
                $items[] = array('name' => $item->get_name());
            }
            $order_history[] = array('order_id' => $order->get_id(), 'date' => $order->get_date_created()->date('Y-m-d'), 'total' => $order->get_total(), 'status' => $order->get_status(), 'items' => $items);
        }
    ...
    public function get_all_courses() {
        if (!function_exists('tutor_utils') || !function_exists('wc_get_product')) {
            return new WP_REST_Response(array('message' => 'Required plugins (Tutor LMS, WooCommerce) are not active.'), 500);
        }
        $args = array('post_type' => 'courses', 'post_status' => 'publish', 'posts_per_page' => -1);
    ...
    public function get_my_courses() {
        if (!function_exists('tutor_utils')) {
            return new WP_REST_Response(array('message' => 'Tutor LMS is not active.'), 500);
        }
    ...
    public function get_course_details(WP_REST_Request $request) {
        ...
        if (!tutor_utils()->is_enrolled($course_id, $user_id)) {
            return new WP_REST_Response(array('message' => 'You are not enrolled in this course.'), 403);
        }
                ...
                if ($contents->have_posts()) {
                    while ($contents->have_posts()) {
                        ...
                        if (get_post_type() === 'tutor_quiz') {
                            $quizzes_data[] = array('id' => get_the_ID(), 'title' => get_the_title());
                        } else {
                            $lessons_data[] = array('id' => get_the_ID(), 'title' => get_the_title());
                        }
                    }
                }
                $topics_data[] = array('id' => $topic_id, 'title' => get_the_title(), 'lessons' => $lessons_data, 'quizzes' => $quizzes_data);
            }
            wp_reset_postdata();
        }
        $response_data = array(
    ...
    public function get_lesson_details(WP_REST_Request $request) {
        ...
        if (!$course_id || !tutor_utils()->is_enrolled($course_id, $user_id)) {
            return new WP_REST_Response(array('message' => 'You do not have access to this lesson.'), 403);
        }
    ...
    public function initiate_payment(WP_REST_Request $request) {
        if (!function_exists('wc_create_order') || !function_exists('tutor_utils')) {
            return new WP_REST_Response(array('success' => false, 'message' => 'WooCommerce or Tutor LMS is not active.', 'error_code' => 'plugin_inactive'), 500);
        }
        ...
        if (empty($course_id) || empty($phone_number_raw)) {
            return new WP_REST_Response(array('success' => false, 'message' => 'Course ID and phone number are required.', 'error_code' => 'missing_params'), 400);
        }
        if (!preg_match('/^(6|9)[0-9]{8}$/', $phone_number_raw)) {
            return new WP_REST_Response(array('success' => false, 'message' => 'Invalid phone number format.', 'error_code' => 'invalid_phone'), 400);
        }

        $user_id = get_current_user_id();
        if (tutor_utils()->is_enrolled($course_id, $user_id)) {
            return new WP_REST_Response(array('success' => false, 'message' => 'You are already enrolled in this course.', 'error_code' => 'already_enrolled'), 400);
        }

        $product_id = get_post_meta($course_id, '_tutor_course_product_id', true);

        if (empty($product_id)) {
            return new WP_REST_Response(array('success' => false, 'message' => 'This course is not available for purchase.', 'error_code' => 'product_not_found'), 404);
        }
        ...
        try {
            ...
            if (isset($response_data['responseCode']) && $response_data['responseCode'] == '2001') {
                ...
                $order->save();
                return new WP_REST_Response(array('success' => true, 'message' => 'Payment successful. You are now enrolled.', 'order_id' => $order_id), 200);
            } else {
            ...
            return new WP_REST_Response(array('success' => false, 'message' => $e->getMessage(), 'error_code' => 'payment_failed'), 400);
        }
    }
}
